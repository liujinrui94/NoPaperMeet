...
public class SavePdf {
     ...
     * 将图片加入PDF并保存
     */
    public void addText(Boolean is) {
        try {
...
//            img.setAbsolutePosition(rectangle.getWidth() * widthScale, rectangle.getHeight() * heightScale);
            if (is) {
                img.scaleAbsolute(557 * 1.0f * density / scale * rectangle.getWidth() / (bitmap.getWidth() / 2), 363 * 1.0f * density / scale * rectangle.getWidth() / (bitmap.getWidth() / 2));
                img.setAbsolutePosition(rectangle.getHeight() * heightScale,rectangle.getWidth() * widthScale );
            }else {
                img.scaleAbsolute(363 * 1.0f * density / scale * rectangle.getWidth() / (bitmap.getWidth() / 2), 557 * 1.0f * density / scale * rectangle.getWidth() / (bitmap.getWidth() / 2));
                //这里设置image相对PDF左下角的偏移量，我的做法是得到放大后位置相对于整个PDF的百分比再乘PDF的大小得到他的相对偏移位置
                img.setAbsolutePosition(rectangle.getWidth() * widthScale, rectangle.getHeight() * heightScale);
            }
            over.addImage(img);
